...
test("POST /api/v1/audit writes an entry; GET /api/v1/audit retrieves it", async (t) => {
  ...
  const listRes = await app.inject({
    method: "GET",
    url: `/api/v1/audit?actor_user_id=${actorId}`,
...
test("GET /api/v1/audit with no matching filter returns an empty array", async (t) => {
  ...
  const res = await app.inject({
    method: "GET",
    url: "/api/v1/audit?actor_user_id=nonexistent-actor-xyz",
    headers: await testAuth.authHeader({ sub: "admin-1", roles: ["SuperAdmin"] }),
  });
...
test("GET /api/v1/audit rejects a non-SuperAdmin role with 403", async (t) => {
  const db = createDb(DATABASE_URL);
  const testAuth = await createTestAuth();
  const app = buildApp({ db, auth: testAuth.authOptions, logger: false });
  t.after(async () => {
