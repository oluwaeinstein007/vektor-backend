...
test("POST/GET/DELETE /api/v1/geofence-zones round-trips a zone", async (t) => {
  ...
  const listRes = await app.inject({ method: "GET", url: "/api/v1/geofence-zones" });
  assert.equal(listRes.statusCode, 200);
  const zones = listRes.json() as Array<{ zone_id: string; name: string }>;
  assert.ok(zones.some((z) => z.zone_id === zone_id && z.name === name));

  const deleteRes = await app.inject({ method: "DELETE", url: `/api/v1/geofence-zones/${zone_id}` });
  assert.equal(deleteRes.statusCode, 204);

  const deleteAgain = await app.inject({ method: "DELETE", url: `/api/v1/geofence-zones/${zone_id}` });
  assert.equal(deleteAgain.statusCode, 404);
});

test("POST /api/v1/alerts/:id/actions acknowledges an alert and logs the action", async (t) => {
  const db = createDb(DATABASE_URL);
  const app = buildApp({ db, logger: false });
  t.after(() => app.close());
  t.after(() => db.$client.end());
  ...
  const res = await app.inject({
    ...
    payload: { action: "ACKNOWLEDGE", operator_id: "operator-42" },
  });
  assert.equal(res.statusCode, 200);
  const body = res.json();
  assert.equal(body.status, "ACKNOWLEDGED");

  const listRes = await app.inject({ method: "GET", url: "/api/v1/alerts" });
...
test("POST /api/v1/alerts/:id/actions 404s for a nonexistent alert", async (t) => {
  ...
  const res = await app.inject({
    ...
    payload: { action: "DISMISS", operator_id: "operator-1" },
  });
