...
test("POST /api/v1/reports then GET /api/v1/reports/:id returns a real generated PDF", async (t) => {
  ...
  const app = buildApp({ db, storageDir: dir, logger: false });
  t.after(() => app.close());
  t.after(() => rm(dir, { recursive: true, force: true }));
  t.after(() => db.$client.end());

  const createRes = await app.inject({ method: "POST", url: "/api/v1/reports", payload: { format: "PDF" } });
  assert.equal(createRes.statusCode, 200);
  const created = createRes.json() as { report_id: string; status: string };
  assert.equal(created.status, "COMPLETE");

  const getRes = await app.inject({ method: "GET", url: `/api/v1/reports/${created.report_id}` });
  assert.equal(getRes.statusCode, 200);
  assert.equal(getRes.headers["content-type"], "application/pdf");
  const buffer = getRes.rawPayload;
  assert.equal(buffer.subarray(0, 5).toString("ascii"), "%PDF-");

  const notFound = await app.inject({ method: "GET", url: `/api/v1/reports/${randomUUID()}` });
...
test("GET /api/v1/export returns real CSV and GeoJSON for entities in range", async (t) => {
  ...
  try {
    ...
    const csvRes = await app.inject({ method: "GET", url: `/api/v1/export?format=csv&from=${from}&to=${to}` });
    ...
    const geojsonRes = await app.inject({ method: "GET", url: `/api/v1/export?format=geojson&from=${from}&to=${to}` });
